...
-   Box: the middle of data (25%-75%) inter qu

-   Whiskers: smallest and largest values within 1.5 × IQR
...
ggplot(pokedata, aes(x = fct_reorder(type_1, hp, median), y = hp)) +
  geom_boxplot() +
  geom_point(with = 0.2, alpha = 0.5) # try jitter, width of 0.2

ggplot(pokedata, aes(x = type_1, y = hp)) +
  geom_boxplot() +
  facet_wrap(~generation) + # try face_grid()
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) # note code to rotate labels
```
...
```{r}
ggplot(data = na.omit(pokedata), aes(x = type_2, fill = type_2)) +
  geom_bar() +
  facet_wrap(~type_1) +
    theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 7)) # note code to rotate labels
...
-   Where do we see a lot of variability?
    ...
    Fair cut show most variability if it include the outlier. If it doesn't put the outlier, Premium has a lot of variability.

-   Is there a relationship between carat and cut?

Diamonds with high cut grade, carat tend to have smaller carat size.
...
The factor with the most influence on price is \_carat\_\_\_. There may be a trade-off between `carat` and `cut`. Perhaps larger carat diamonds can be profitably sold even with a low quality cut, for example.
...
Answer: For me a boxplot because it directly show the relationship between two variable price and carat. I could easily see the patterns and trends.

...
gebtoo_data_from_url <- read_csv(url_gentoo)
```
...
```{r, message=FALSE}
ggplot(data = penguins_data, aes(x = flipper_length_mm, fill = species)) +
  geom_histogram(alpha = 0.5) + ## can turn off the waring, waring = False
  theme_bw() + ## theme black and wite
  scale_fill_manual(values = c("darkorange","darkorchid","cyan4")) + ## chose the color, alphabeticry order
...
```{r, message=FALSE}
ggplot(data = penguins_data, aes(x = flipper_length_mm, fill = species)) +
  geom_histogram() + ## can turn off the waring, waring = False
  theme_bw() + ## theme black and wite
  scale_fill_viridis_d() +
  labs(title = "Palmer Penguins, Distribution of Clumen depth",
       subtitle = "3 penguin species observed on three islands in the Palmer Archipelago, Antarctica",
       x = "Culmen depth (mm)",
       y = "Number of Penguins")
```
...
ggplot(data = penguins_data, aes(x = culmen_depth_mm, y = body_mass_g, color = species, shape = island)) +
  geom_point() +
  theme_bw() + scale_fill_viridis_d(option = "mako") +
  labs(title = "Palmer Penguins, Distribution of Clumen depth",
       subtitle = "3 penguin species observed on three islands in the Palmer Archipelago, Antarctica",
       x = "Culmen depth (mm)",
       y = "Bodymass (g)")

...
```{r, messages=FALSE}
unique(filter$island) # an example of how to use 
#unique(penguins$culmen_depth_mm)

filter <- penguins %>%
  filter(sex == "FEMALE", culmen_length_mm >= 39, island %in% c("Dream", "Torgerson"))
...
across <- penguins %>%
  ...
  group_by(species) %>%
  summarise(across(where(is.numeric), max, np.rm = TRUE))
...
```{r, message=FALSE, warning=FALSE}
final_boss <- penguins %>%
  # Filter to keep only female penguins with a known flipper_length_mm (not NA) that are either on Dream or Biscoeisland.
  filter(sex == "FEMALE", flipper_length_mm >= 0, island %in% c("Biscoe", "Dream")) %>%
  #Select just these columns: species, island, flipper_length_mm, and body_mass_g
  select(species, island, flipper_length_mm, body_mass_g) %>%
  #Mutate a new variable called flipper_rank:
#If flipper_length_mm is less than 190, assign "short",
#If flipper_length_mm is between 190 and 210, assign "medium",
#If greater than 210, assign "long"
  ...
  #Group by species, island, and flipper_rank, and summarize the average body mass (name this column avg_mass_g), rounded to the nearest whole number.
  group_by(species, island, flipper_rank) %>%
  summarise(avg_mass_g = round(mean(body_mass_g,na.rm = TRUE)))
